    ...
    <div className="min-h-screen" style={{ background: 'var(--background)', color: 'var(--text-primary)' }}>
      ...
      <div className="sticky top-0 z-10 mt-3" style={{ background: 'rgba(var(--background-rgb), 0.9)', backdropFilter: 'blur(10px)' }}>
        <div className="max-w-7xl mx-auto px-6 py-3 overflow-x-auto">
          <div className="flex items-center justify-between">
            ...
            {/* 桌面端搜索框 */}
            <div className="relative flex-shrink-0 ml-4 hidden sm:block">
              <div className="relative">
                <input 
                  ...
                  className="w-48 md:w-64 py-1.5 pl-9 pr-3 text-sm rounded-md transition-all duration-300 focus:ring-2 focus:ring-opacity-50 focus:outline-none" 
                  style={{ 
                    background: 'var(--card-bg)', 
                    border: `1px solid ${isSearchFocused ? 'var(--accent)' : 'var(--card-border)'}`,
                    color: 'var(--text-primary)',
                    boxShadow: isSearchFocused ? '0 0 0 2px rgba(var(--accent-rgb), 0.2)' : '0 1px 3px rgba(0,0,0,0.05)',
                  }}
                />
            ...
            {/* 移动端搜索按钮 */}
            <div className="relative flex-shrink-0 ml-2 block sm:hidden">
              <button 
                onClick={() => setShowMobileSearch(!showMobileSearch)}
                className="p-1.5 rounded-md transition-all duration-300 hover:bg-opacity-90"
                style={{ 
                  background: 'var(--card-bg)', 
              ...
              <div className="py-2 block sm:hidden">
                <div className="relative">
                  <input 
                    ...
                    onFocus={() => setIsSearchFocused(true)}
                    onBlur={() => setIsSearchFocused(false)}
                    className="w-full py-1.5 pl-9 pr-3 text-sm rounded-md transition-all duration-300 focus:ring-2 focus:ring-opacity-50 focus:outline-none" 
                    style={{ 
                      background: 'var(--card-bg)', 
                      border: `1px solid ${isSearchFocused ? 'var(--accent)' : 'var(--card-border)'}`,
                      color: 'var(--text-primary)',
      ...
      {/* Main Content */}
       <main className="max-w-7xl mx-auto px-6 py-8">
         ...
         {showSearchResults && (
           <div className="space-y-6 mb-10">
             <div className="flex items-center justify-between">
               ...
               >
                 <span className="relative z-10">搜索结果</span>
                 <span 
             ...
             {searchResults.length === 0 ? (
               ...
               <div className="space-y-8">
                 {searchResults.map((result, resultIndex) => (
                   ...
                   >
                     <h3 className="text-lg font-semibold mb-6 relative inline-block" style={{ color: 'var(--text-primary)' }}>
         ...
         {/* 主要内容区域 */}
         <div className={`space-y-10 ${showSearchResults ? 'hidden' : ''}`}>
           {categories
