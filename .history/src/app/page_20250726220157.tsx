      ...
      {/* Main Content */}
       <main className="max-w-7xl mx-auto px-6 py-8">
         <div className="space-y-10">
           {searchTerm.trim() !== "" && categories
             .filter(category => (selectedCategory === "全部" || category.title === selectedCategory) && categoryHasMatchingLinks(category))
             .length === 0 ? (
             <div className="text-center py-10">
               <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto mb-4 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: 'var(--text-secondary)' }}>
                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
               </svg>
               <h3 className="text-lg font-medium mb-2" style={{ color: 'var(--text-primary)' }}>未找到匹配的结果</h3>
               <p className="text-sm opacity-70" style={{ color: 'var(--text-secondary)' }}>尝试使用其他关键词搜索</p>
             </div>
           ) : (
             categories
             .filter(category => (selectedCategory === "全部" || category.title === selectedCategory) && categoryHasMatchingLinks(category))
             ...
             >
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                 {filterLinks(category).map((link, linkIndex) => (
                   <a
                     key={linkIndex}
                     href={link.url}
                     target="_blank"
