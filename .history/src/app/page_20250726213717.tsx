export default function Home() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)', color: 'var(--text-primary)' }}>
      ...
      {/* Category Navigation */}
      <div className="sticky top-0 z-10" style={{ background: 'rgba(var(--header-bg-rgb), 0.7)', borderBottom: '1px solid var(--card-border)', backdropFilter: 'blur(8px)' }}>
        <div className="max-w-7xl mx-auto px-6 py-2 overflow-x-auto">
          <div className="flex items-center">
            <div className="flex-shrink-0 mr-3">
              <div className="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: 'var(--accent)' }}>
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
                <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>分类导航</span>
              </div>
            </div>
            <div className="flex space-x-2.5 overflow-x-auto pb-1 scrollbar-thin flex-grow">
              {categories.map((category, index) => (
                <a 
                  key={index} 
                  href={`#category-${index}`}
                  className="text-xs font-medium whitespace-nowrap px-3 py-1 rounded transition-all duration-200 hover:bg-opacity-90 hover:translate-y-[-1px] flex-shrink-0"
                  style={{ 
