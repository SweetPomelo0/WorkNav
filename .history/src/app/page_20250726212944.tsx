export default function Home() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)', color: 'var(--text-primary)' }}>
      ...
      {/* Category Navigation */}
      <div className="sticky top-0 z-10 py-3" style={{ background: 'var(--header-bg)', borderBottom: '1px solid var(--card-border)' }}>
          ...
          <div className="flex space-x-4 pb-1">
            {categories.map((category, index) => (
              <a 
                key={index} 
                href={`#category-${index}`}
                className="text-sm whitespace-nowrap px-3 py-1.5 rounded-full transition-all duration-200 hover:bg-opacity-80"
                style={{ 
